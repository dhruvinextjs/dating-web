...
export default function HeaderAfterLogin() {
  ...
  const linkClass = (path) =>
    pathname === path
      ? "text-[#7B00F7]"
      : "text-[#333] hover:text-[#7B00F7] transition";

  return (
    <header className="w-full bg-[#ffffff] shadow-sm fixed top-0 left-0 z-60">
      <div className="p-5 max-w-7xl mx-auto flex items-center justify-between relative">
        ...
        <nav className="hidden lg:flex items-center gap-6 text-[#000D23]">
          ...
          <Link href={"/like"} className={linkClass("/like")}>
            Likes
          </Link>
      ...
      >
        <div className="flex flex-col gap-4 text-center px-5 py-5 text-[#333]">
          ...
          <Link href="/like" className={linkClass("/like")} onClick={() => setOpen(false)}>Likes</Link>
